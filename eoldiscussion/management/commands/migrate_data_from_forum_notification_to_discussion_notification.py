...
def import_discussions_notification():
    ...
    try:
        ...
        # Raise exception
        raise CommandError(f"Error while copying discussions {e} from import_discussions_notification function")
...
def import_discussions_notification_user():
    ...
    try:
        ...
        logger.error(f'Exception happens: {e}')
        # Raise exception
        raise CommandError(f"Error while copying discussion notifications {e} from import_discussions_notification_user function") 
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not(apps.is_installed('eol_forum_notifications')) or not(apps.is_installed('eoldiscussion')):
            ...
            raise CommandError(f"One of the apps, eol_forum_notifications or eoldiscussion, isn't installed, so I can't continue with the commands.")
        ...
        with transaction.atomic():
            try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"""
                                ...
                                A total of {discussion_notification_import_result[0]} EolForumNotificationsDiscussions objects were found.
                                A total of {discussion_notification_import_result[1]} objects already exist in EolDiscussionXBlockNotification model.
                                A total of {discussion_notification_import_result[2]} objects could be copied to the EolDiscussionXBlockNotification model.

                                A total of {notification_user_import_result[0]} EolForumNotificationsUser objects were found.
                                A total of {notification_user_import_result[1]} objects already exist in EolDiscussionXBlockNotificationUser model.
                                A total of {notification_user_import_result[2]} objects could be copied to the EolDiscussionXBlockNotificationUser model.
                            """
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"""
                                A total of {discussion_notification_import_result[0]} EolForumNotificationsDiscussions objects were found.
                                A total of {discussion_notification_import_result[1]} objects already exist in EolDiscussionXBlockNotification model.
                                A total of {discussion_notification_import_result[2]} objects have been copied to the EolDiscussionXBlockNotification model.

                                A total of {notification_user_import_result[0]} EolForumNotificationsUser objects were found.
                                A total of {notification_user_import_result[1]} objects already exist in EolDiscussionXBlockNotificationUser model.
                                A total of {notification_user_import_result[2]} objects have been copied to the EolDiscussionXBlockNotificationUser model.
                            """
                        )
                    )
            except Exception as e:
                ...
                logger.error(f'Exception happens: {e}')
                # Raise exception
                raise CommandError(f"Error while copying data: {e}")
            if  dry_run:
                transaction.set_rollback(True)
